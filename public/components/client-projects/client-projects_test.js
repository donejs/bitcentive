...
QUnit.asyncTest('Add an OS project to a monthly client project', function() {
    ...
    const monthlyOSProjects = vm.contributionMonth.monthlyClientProjects[0].monthlyOSProjects;
    vm.toggleUseProject(vm.contributionMonth, monthlyOSProjects, newMonthlyOSProject).then( () => {
        QUnit.equal(vm.contributionMonth.monthlyClientProjects[0].monthlyOSProjects.length , 2);
        QUnit.equal(vm.contributionMonth.monthlyClientProjects[0].monthlyOSProjects[1].osProjectRef._id , newMonthlyOSProject.osProjectRef);
        QUnit.start();
    });
});
...
QUnit.asyncTest('Remove an OS project to a monthly client project', function() {
    ...
    const monthlyOSProjects = vm.contributionMonth.monthlyClientProjects[0].monthlyOSProjects;
    vm.toggleUseProject(vm.contributionMonth, monthlyOSProjects, newMonthlyOSProject).then(() => {
        QUnit.equal(vm.contributionMonth.monthlyClientProjects[0].monthlyOSProjects.length , 0);
        QUnit.start();
    });
});
